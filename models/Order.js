...
const orderItemSchema = new mongoose.Schema({
  ...
  // ── Parda-specific fields (only for products starting with "Parda") ──
  // These are optional — only populated when the product is a Parda/curtain
  width: {
    type: Number,
    min: 0
  },
...
const orderSchema = new mongoose.Schema({
  ...
  // Fast Order flag (POS counter sale — no client, no delivery tracking, instant completion)
  isFastOrder: {
    type: Boolean,
    default: false,
    index: true
  ...
  // Used as deduplication key — if two sync requests carry the same offlineId,
